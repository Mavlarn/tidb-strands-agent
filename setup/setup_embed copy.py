...
try:
    ...
    print(f"Error opening customer service table: {e}")
...
if pr_table is None:
    pr_table = db.create_table(schema=ProductReview, mode="overwrite")
if cs_table is None:
    cs_table = db.create_table(schema=CSRecords, mode="overwrite")
...
list2 = cs_table.search("ZenPhone", search_type="hybrid").text_column("product_name").limit(3).to_list()
